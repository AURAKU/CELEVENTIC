...
function resolveShareLink(sharePath: string, appUrl: string): string {
  const cleaned = collapseDuplicateAbsoluteUrl(sharePath.trim());
  if (/^https?:\/\//i.test(cleaned)) {
    return collapseDuplicateAbsoluteUrl(sanitizePublicUrl(cleaned, appUrl));
  }
  const path = cleaned.startsWith("/") ? cleaned : `/${cleaned}`;
  return `${appUrl}${path}`;
}

function withGuestToken(link: string, token: string): string {
  try {
    const url = new URL(link);
    url.searchParams.set("guest", token);
    return url.toString();
  } catch {
    return `${link}${link.includes("?") ? "&" : "?"}guest=${encodeURIComponent(token)}`;
  }
}
...
export class InvitationSharingService {
  buildWhatsAppPack(options: {
    ...
    const dateLine = options.eventDate
      ? new Date(options.eventDate).toLocaleDateString("en-GB", {
          weekday: "short",
          month: "short",
          day: "numeric",
          year: "numeric",
    ...
    const generalTextFr = ensureSingleShareUrl(
      `✨ *${options.eventTitle}*\n\n` +
    ...
    return {
      ...
      shareLink: link,
      guestPersonalizedLink: (token, _name) => withGuestToken(link, token),
      guestMessage: (name, token) => {
        const personal = withGuestToken(link, token);
        const host = options.hostName?.trim() || "Your hosts";
        return (
          ensureSingleShareUrl(
