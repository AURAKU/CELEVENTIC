...
export function AdminInvitationOrdersClient() {
  ...
  const load = useCallback(async () => {
    ...
    if (d.success) {
      ...
      setOrders((Array.isArray(list) ? list : []).map((o: AdminOrder) => ({
        ...o,
  ...
  }, [search, statusFilter, productionFilter, appendToParams]);

  useEffect(() => {
    resetPage();
  }, [search, statusFilter, productionFilter, resetPage]);
                  ...
                  <TabsContent value="actions" className="space-y-3 mt-4">
                    ...
                    <div>
                      ...
                      <ImageUploadCropper
                        ...
                        onUploaded={async (r) => {
                          setPreviewUrl(r.url);
                          await productionAction({ action: "upload_preview", previewUrl: r.url });
                        }}
                      />
                      {previewUrl && (
                        <Button
                          ...
                          className="mt-2"
                          variant="outline"
                          onClick={() => productionAction({ action: "upload_preview", previewUrl })}
                        >
                          Re-send preview to customer
                        </Button>
                      )}
                    </div>
