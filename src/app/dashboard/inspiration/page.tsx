...
export default function InspirationPage() {
  ...
  const [form, setForm] = useState({ type: "IMAGE" });
  ...
  async function registerUpload(url: string) {
    const res = await fetch("/api/inspiration", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: form.type, url }),
    });
    if (res.ok) {
  ...
  async function handleUpload(e: React.FormEvent) {
    e.preventDefault();
    if (uploadedUrl) await registerUpload(uploadedUrl);
  }
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleUpload} className="space-y-3">
              <ImageUploadCropper
                defaultAspect="3:4"
                buttonLabel="Import & crop design"
                hint="Upload invitation, flyer, or card artwork from your device."
                onUploaded={async (r) => {
                  setUploadedUrl(r.url);
                  await registerUpload(r.url);
                }}
              />
