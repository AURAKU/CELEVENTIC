...
export default function GuestsPage() {
  ...
  const stats = useMemo(() => {
    const counts: Record<string, number> = { INVITED: 0, OPENED: 0, ACCEPTED: 0, DECLINED: 0, MAYBE: 0, CHECKED_IN: 0 };
    ...
    const noResponse = total - responded - counts.OPENED;
    return { counts, total, noResponse: Math.max(0, noResponse) };
  }, [guests]);

  const filtered = useMemo(() => {
    if (filter === "all") return guests;
    if (filter === "NO_RESPONSE") {
      return guests.filter((g) => !["ACCEPTED", "DECLINED", "MAYBE", "CHECKED_IN", "OPENED"].includes(g.status));
    }
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Users className="h-5 w-5" /> Guest List ({filtered.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {!eventId ? (
