...
function WalletPageInner() {
  ...
  const loadWallet = useCallback(
    async (pageNum = txPage) => {
      if (!eventId) return;
      setLoading(true);
      const res = await fetch(
        `/api/wallet?eventId=${encodeURIComponent(eventId)}&page=${pageNum}&limit=${TX_PER_PAGE}`
      );
  ...
  useEffect(() => {
    if (eventId) void loadWallet(txPage);
  }, [eventId, txPage, loadWallet]);
  ...
  const txMeta = data?.transactionsMeta ?? {
    total: data?.wallet?.transactions?.length ?? 0,
    page: txPage,
    limit: TX_PER_PAGE,
    pages: 1,
    hasMore: false,
        ...
        <>
          <div id="wallet-revenue" className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 scroll-mt-24">
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-2xl font-bold">{formatCurrency(data.wallet.balance)}</p>
            ...
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-2xl font-bold">{formatCurrency(data.wallet.revenue)}</p>
            ...
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-2xl font-bold">{formatCurrency(data.wallet.expenses)}</p>
            ...
            <Card>
              <CardContent className="p-6 text-center">
                <DollarSign
                  className={`h-6 w-6 mx-auto mb-2 ${(pl?.netProfit ?? 0) >= 0 ? "text-green-600" : "text-red-500"}`}
                />
                <p className="text-2xl font-bold">{formatCurrency(pl?.netProfit ?? 0)}</p>
                ...
                <form onSubmit={addExpense} className="space-y-3">
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      required
                      disabled={!eventId}
                      placeholder="Venue, Catering, Decor..."
                    />
                  </div>
                  <div className="space-y-1">
                  ...
                  <>
                    {data.wallet.transactions.map((t, i) => (
                      <div key={i} className="flex justify-between text-sm py-2 border-b gap-2">
                        ...
                        <span
                          className={`shrink-0 ${Number(t.amount) < 0 ? "text-red-600" : "text-green-600"}`}
                        >
          ...
          <Card>
            <CardContent className="p-4 text-sm flex flex-wrap gap-4">
              <span>
                Contributions: <strong>{formatCurrency(data.contributions.total)}</strong> from{" "}
                {data.contributions.count} donors
              </span>
              {pl && (
                <span>
                  Budget position:{" "}
                  <strong className={pl.netProfit >= 0 ? "text-green-600" : "text-red-600"}>
