...
export function BlockEditor({ orderId, blocks, available, context, onChange }: BlockEditorProps) {
  ...
  function startEdit(block: InvitationBlockDto) {
    ...
    setEditForm({
      ...
      galleryUrls: (block.galleryItems ?? []).map((g) => g.url).join("\n"),
    });
  }

  async function saveEdit(blockId: string) {
    setSaving(true);
    const galleryUrls = editForm.galleryUrls.split("\n").map((u) => u.trim()).filter(Boolean);
            ...
            {editingId === block.id && (
              <div className="mt-4 pt-4 border-t space-y-3">
                ...
                {(block.blockType === "GALLERY" || block.blockType === "MEMORIAL_GALLERY") && (
                  <div><Label>Gallery URLs (one per line)</Label><Textarea rows={3} value={editForm.galleryUrls} onChange={(e) => setEditForm({ ...editForm, galleryUrls: e.target.value })} /></div>
                )}
                <div>
                  <Label>Style</Label>
                  <select
