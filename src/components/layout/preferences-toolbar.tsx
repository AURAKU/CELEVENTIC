...
export function PreferencesToolbar({
  ...
  return (
    <div
      className={cn(
        "inline-flex items-center gap-2 rounded-2xl border border-slate-200/80 bg-white/95 backdrop-blur-xl shadow-[0_4px_24px_rgba(15,23,42,0.08)] p-1.5",
        vertical && "flex-col items-stretch",
        className
      )}
    >
      {showLanguage && <LanguageSwitcher compact showFlags allowedLocales={allowedLocales} />}
      {showLanguage && showCurrency && (
        <span className={cn("bg-slate-200", vertical ? "h-px w-full" : "w-px h-5")} aria-hidden />
      )}
      {showCurrency && <CurrencySwitcher compact showFlags />}
    </div>
  );
}
